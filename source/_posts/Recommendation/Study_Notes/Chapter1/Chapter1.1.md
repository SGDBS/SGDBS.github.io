...
$$200050 + 199980 + 200010 + 199950 + 200010 = 1{,}000{,}000$$
...
```
执行这条 SQL 的目的：是为了知道“点击了这个商品的人，到底是哪个年龄段的？”
这是特征工程中最关键的一步。有了 age_group，模型才能学习到：“iPhone 17 主要是 20-30 岁的青年人在点击”，从而实现精准推荐。
...
**热点问题**：`User_999` 是一个爬虫账号，在表 A 中产生了 **200 万条**点击。单 Task 内存上限为 **50 万条**。若直接 Shuffle Join，处理 `User_999` 的 Task 必然 OOM。
...
属性表中 `User_999` 的那 1 行记录**复制 4 份**，分别命名为 `User_999_0` 到 `User_999_3`，确保每个加盐 Key 都能找到匹配项。
...
随机化映射后，如何保证模型认为 `Unknown_0` 和 `Unknown_9` 是同一个东西？
...
## 5.流处理框架选型 (Spark Streaming vs. Flink)
...
2.  **复杂的窗口计算**：Flink 支持滑动窗口、滚动窗口及**会话窗口 (Session Window)**，非常适合分析用户在单次 App 打开期间的连续行为流。
...
#### 💡 面试加分金句
> “Spark Streaming 是用**批**的思维去模拟**流**，而 Flink 是用**流**的思维去包含**批**。在实时推荐这种对‘时效性’和‘细粒度状态’有极致追求的场景下，Flink 的原生流架构具备不可替代的代差优势。”