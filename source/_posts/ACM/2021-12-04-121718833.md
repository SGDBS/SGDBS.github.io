...
Kruskal
Kruskal重构树的方法当然也能解决。  
 [参考了这个dalao的博客](https://www.cnblogs.com/wizarderror/p/12187451.html)

### 主要思想

建一个新的树

* 按照Kruskal算法依次遍历边,**当要进行合并时,再执行以下操作**
* 把最小生成树上的边
  (
  u
  ,
...
{
    for(int i = tot; i >= 0; --i) //这个地方要倒序,因为$tot$是根节点
        if(!depth[i])
            dfs(i);
