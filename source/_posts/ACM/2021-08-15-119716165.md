...
n
n
n个盒子，每个盒子里装有一个球，它可能是黑色或者白色的概率均为
1
/
2
1/2
...
C
C
C的价值来获得**剩下的所有盒子中剩余的黑色球数量和白色球数量**。还可以花费
w
[
i
]
...
i
i
i​个盒子才知道所有盒子球的颜色的概率。\*\*也就是说，我们打开第
i
−
1
i-1
i−1个盒子的时候，仍然不知道，打开了第
i
i
i个盒子的时候，刚好就知道了。说明：
...
i
i
i个盒子颜色与剩下的相同，那么我们在打开第
i
−
1
i-1
i−1个盒子的时候就已经知道剩下盒子的颜色了。
...
−
$$
$ans=max(\sum\_{i=0}^{n}sum[i]\*(1/2^{n-i}),$ sum[n])
$$
